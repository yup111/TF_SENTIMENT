...
def embd(X, ndim, scope='embedding'):
    with tf.variable_scope(scope):
        embd = tf.get_variable(
            "x", [hps.nvocab, ndim], initializer=load_params)
...
def mlstm(inputs, c, h, M, ndim, scope='lstm', wn=False):
    ...
    with tf.variable_scope(scope):
        z = tf.get_variable("z", [nin, ndim * 4], initializer=load_params)
        initializer = tf.random_normal_initializer(mean=0., stddev=1.,)
        wx = tf.get_variable("wx", [nin, ndim * 4], initializer=initializer)

        x = tf.get_variable("x", [nin, ndim * 4], initializer=load_params)
        initializer = tf.random_normal_initializer(mean=0., stddev=1.,)
        wh = tf.get_variable("wh", [ndim, ndim * 4], initializer=initializer)
        wmx = tf.get_variable("wmx", [nin, ndim], initializer=load_params)
        wmh = tf.get_variable("wmh", [ndim, ndim], initializer=load_params)
        b = tf.get_variable("y", [ndim * 4], initializer=load_params)
        if wn:
...
if __name__ == '__main__':
    ...
    print('sentiment')
    print(sentiment)
    # predictions_human_readable = np.column_stack((np.array(text), sentiment))
